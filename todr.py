...
# aurs04u = Car("Audi RS6 plus", 2004, 177, 4.1, 85, "4WD", "PER", 2, True, True)
#
# # BMW
# bmm178 = Car("BMW M1", 1978, 163, 5.5, 75, "RWD", "PER", 1, False, False)
# bmm596 = Car("BMW M5", 1996, 155, 4.9, 79, "RWD", "PER", 2, True, True)
# bmm596u = Car("BMW M5", 1996, 159, 4.6, 82, "RWD", "PER", 2, True, True)
# bmx516 = Car("BMW X5 40e (PHEV)", 2016, 130, 6.8, 69, "4WD", "ALL", 3, True, True)
# bmx516u = Car("BMW X5 40e (PHEV)", 2016, 141, 5.4, 74, "4WD", "ALL", 3, True, True)
# bmz800 = Car("BMW Z8", 2000, 155, 4.5, 77, "RWD", "PER", 1, True, True)
# bmz800u = Car("BMW Z8", 2000, 164, 3.8, 85, "RWD", "PER", 1, True, True)
#
# # Chevrolet
# chtr06 = Car("Chevrolet Trailblazer SS", 2006, 142, 5.6, 62, "4WD", "ALL", 3, True, True)
# chtr06u = Car("Chevrolet Trailblazer SS", 2006, 154, 4.5, 67, "4WD", "ALL", 3, True, True)
# chtr06a = Car("Chevrolet Trailblazer SS", 2006, 142, 5.6, 62, "4WD", "ALL", 3, True, True)
# chca16 = Car("Chevrolet Camaro Z/28", 2016, 187, 4.0, 87, "RWD", "PER", 2, True, True)
# chca16u = Car("Chevrolet Camaro Z/28", 2016, 190, 3.6, 90, "RWD", "PER", 2, True, True)
#
# # Citroen
#
# # Ford
# fofo12 = Car("Ford Focus ST", 2017, 150, 6.5, 84, "FWD", "PER", 2, True, True)
# fofo12u = Car("Ford Focus ST", 2017, 150, 6.5, 84, "FWD", "PER", 2, True, True)
# fors84 = Car("Ford RS200", 1984, 142, 4.8, 81, "4WD", "PER", 1)
# fors84u = Car("Ford RS200", 1984, 145, 4.5, 84, "4WD", "PER", 1)
#
# # Honda
# hons90 = Car("Honda NSX", 1990, 168, 5.7, 84, "RWD", "PER", 1)
# hons90u = Car("Honda NSX", 1990, 172, 5.1, 87, "RWD", "PER", 1)
#
# # Hummer
...
# nimu02u = Car("Nissan Murano GT-C", 2002, 148, 6.5, 83, "4WD", "ALL", 3)
#
# # Subaru
...
# rq20 = [fors84]; rq21 = [mecl15, suwr14]; rq22 = [aurs04]; rq23 = [chca16]
# rq25 = [mc1211]
#
# # Upgraded spec
# rq13u = []
# rq14u = [auqu90, recl09, vo8597]
# rq15u = [bmm178, nixt16, sule16u]
...
# rq20u = [fors84u]; rq21u = [mecl15u, suwr14u]; rq22u = [aurs04u]; rq23u = [chca16u]
# rq25u = [mc1211u]
...
# cars.extend(rq20); cars.extend(rq21); cars.extend(rq22); cars.extend(rq23);
# cars.extend(rq25)
# Upgraded spec
# cars.extend(rq13u); cars.extend(rq14u); cars.extend(rq15u); cars.extend(rq16u)
# cars.extend(rq18u); cars.extend(rq19u);
# cars.extend(rq20u); cars.extend(rq21u); cars.extend(rq22u)
# cars.extend(rq23u); cars.extend(rq25u)

#filename = "cardata.csv"
#filename = "cardata_with_upgrades.csv"
filename = "cardata_all.csv"

with open(filename, newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        name, rq, year, ts, acc, grip, drive, tyres, ride = row[0:9]
        if len(row) > 9:
            trac, absbr = row[9:11]
            # Full init
            cars.append(Car(name, rq, year, ts, acc, grip, drive, tyres, ride, trac, absbr))
        else:
            # Partial init
            cars.append(Car(name, rq, year, ts, acc, grip, drive, tyres))
                        ...
                        for subCar in otherCar.doms:
                            ...
                            if len(subCar.doms) > 0:
                                for subSubCar in subCar.doms:
                                    ...
                                    #         if len(subSubSubCar.doms) > 0:
                                    #             for subSubSubSubCar in subSubSubCar.doms:
                                    #                 print("\t\t\t\t\t" + subSubSubSubCar.name)
                                    #                 if len(subSubSubSubCar.doms) > 0:
                                    #                     for subSubSubSubSubCar in subSubSubSubCar.doms:
                                    #                         print("\t\t\t\t\t\t" + subSubSubSubSubCar.name)
                                    #                         if len(subSubSubSubSubCar.doms) > 0:
                                    #                             for subSubSubSubSubSubCar in subSubSubSubSubCar.doms:
                                    #                                 print("\t\t\t\t\t\t\t" + subSubSubSubSubSubCar.name)
